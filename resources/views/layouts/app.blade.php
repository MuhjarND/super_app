                        ...
                        @if($showDesktopModule('library') && ($isSidebarSuperAdmin || $sidebarUser->canAccessLibraryModule()))
                            <li class="nav-section" data-section="perpustakaan">
                                ...
                                <ul class="nav nav-pills flex-column nav-section-menu">
                                    ...
                                    @endif
                                    <li class="nav-item nav-item-sub">
                                        <a href="{{ route('library.loans.index') }}" class="nav-link {{ request()->routeIs('library.loans.*') ? 'active' : '' }}">
                                            <i class="nav-icon fas fa-exchange-alt"></i>
                                            <p>
                                                {{ ($isSidebarSuperAdmin || $sidebarUser->canManageLibraryModule()) ? 'Peminjaman' : 'Peminjaman Saya' }}
                                                @if($sidebarSubmoduleBadge('perpustakaan', ($isSidebarSuperAdmin || $sidebarUser->canManageLibraryModule()) ? 'peminjaman' : 'peminjaman_saya') > 0)
                                                    <span class="right badge badge-danger">{{ $sidebarBadgeText($sidebarSubmoduleBadge('perpustakaan', ($isSidebarSuperAdmin || $sidebarUser->canManageLibraryModule()) ? 'peminjaman' : 'peminjaman_saya')) }}</span>
                                                @endif
                                            </p>
                                        </a>
                                    </li>
                                    @if($isSidebarSuperAdmin || $sidebarUser->canManageLibraryModule())
                                        ...
                                        <li class="nav-item nav-item-sub">
                                            <a href="{{ route('library.fines.index') }}" class="nav-link {{ request()->routeIs('library.fines.*') ? 'active' : '' }}">
                                                <i class="nav-icon fas fa-coins"></i>
                                                <p>
                                                    Denda
                                                    @if($sidebarSubmoduleBadge('perpustakaan', 'denda') > 0)
                                                        <span class="right badge badge-danger">{{ $sidebarBadgeText($sidebarSubmoduleBadge('perpustakaan', 'denda')) }}</span>
                                                    @endif
                                                </p>
                                            </a>
                                        </li>
