...
<div class="card shadow-sm border-0 mb-3">
    <div class="card-body d-flex justify-content-between align-items-center flex-wrap zi-area-top" style="gap:12px;">
        <div>
            <h4 class="mb-1">Master Area ZI</h4>
            <div class="text-muted">Kelola area ZI berdasarkan kelompok Pengungkit, Reform, dan Hasil.</div>
        </div>
        @if($canManage)
...
<div class="card shadow-sm border-0">
    <div class="card-body table-responsive p-0">
        <table class="table table-hover mb-0 zi-area-table">
            <thead><tr><th>Kode</th><th>Kelompok</th><th>Nama Area</th><th>PIC Area</th><th>Status</th><th>Deskripsi</th>@if($canManage)<th style="width:90px;">Aksi</th>@endif</tr></thead>
            <tbody>
                @if($areas->isEmpty())
                    <tr><td colspan="{{ $canManage ? 7 : 6 }}" class="text-center text-muted py-4">Belum ada area ZI.</td></tr>
                @else
                    @foreach($groupedAreas as $groupType => $groupAreas)
                        @if($groupAreas->isNotEmpty())
                            <tr>
                                <td colspan="{{ $canManage ? 7 : 6 }}" class="bg-light font-weight-bold text-uppercase text-muted" style="letter-spacing:.06em; font-size:.74rem;">
                                    {{ \App\ZiArea::groupOptions()[$groupType] ?? ucfirst($groupType) }}
                                </td>
                            </tr>
                        @endif
                        ...
                        <tr>
                            ...
                            <td data-label="Kelompok">{!! $area->group_badge !!}</td>
                        ...
                        <form method="POST" action="{{ route('progress-zi.areas.update', $area) }}">@csrf @method('PUT')<div class="modal-body">
                            ...
                            <div class="form-group"><label>Kelompok</label><select class="form-control" name="group_type" required>@foreach($groupOptions as $value => $label)<option value="{{ $value }}" {{ $area->group_type === $value ? 'selected' : '' }}>{{ $label }}</option>@endforeach</select></div>
...
<div class="modal fade" id="createAreaModal" tabindex="-1"><div class="modal-dialog"><div class="modal-content"><div class="modal-header"><h5 class="modal-title">Tambah Area</h5><button type="button" class="close" data-dismiss="modal"><span>&times;</span></button></div>
    <form method="POST" action="{{ route('progress-zi.areas.store') }}">@csrf<div class="modal-body">
        ...
        <div class="form-group"><label>Kelompok</label><select class="form-control" name="group_type" required>@foreach($groupOptions as $value => $label)<option value="{{ $value }}" {{ $value === \App\ZiArea::GROUP_PENGUNGKIT ? 'selected' : '' }}>{{ $label }}</option>@endforeach</select></div>
