...
class LeaveRequest extends Model
{
    ...
    public function regularLeaveDays()
    {
    ...
    public function balanceDaysForCurrentStatus()
    {
        if (in_array($this->status, [self::STATUS_APPROVED, self::STATUS_COMPLETED], true)) {
            return $this->approvedBalanceDays();
        }

        if (in_array($this->status, [self::STATUS_SUBMITTED, self::STATUS_UNDER_REVIEW, self::STATUS_VERIFIED], true)) {
